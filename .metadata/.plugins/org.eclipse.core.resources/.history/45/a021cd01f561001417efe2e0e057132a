...
//		Eu_norm_share=complex_add(Eu_norm_share,complex_mulcom(d_R[IDC2D(count1,count3,MATRIX_SIZE)],R_Eu_share[count3]));
					Eu_norm_share = complex_add(Eu_norm_share,
							complex_mulcom(
									d_R[IDC2D(count1,count3,MATRIX_SIZE)],
									R_Eu_share[count3]));
				}
...
//__syncthreads();
				...
				for (count1 = 0; count1 < MATRIX_SIZE; count1++) {
					s_potential[d_list[count1] - 1] =
							s_potential_matrix[IDC2D((MATRIX_SIZE-count1-1),Eu_mini_index,pathNum)];
...
//	int *s_sub_index=(int*)calloc(1,rho*pow(M,rho)*sizeof(int));
	...
	cudaHostAlloc((void**) &s_sub_index, rho * pathNum * sizeof(int),
...
//	Eu=(float*)calloc(1,blockNum*sizeof(float));
	...
	cudaHostAlloc((void**) &s_potential_matrix,
			MATRIX_SIZE * blockNum * sizeof(cuComplex), cudaHostAllocDefault);
...
//		error=cudaMemcpy(d_list, list, Nt*(sizeof(int)),cudaMemcpyHostToDevice);
	...
	cudaMemcpyAsync(d_s_sub_index, s_sub_index, rho * (pathNum) * sizeof(int),
...
//	cudaMemcpyAsync(s_potential_matrix,d_s_potential_matrix,MATRIX_SIZE*(pathNum)*sizeof(cuComplex),cudaMemcpyHostToDevice,0);
		FEpath<<<blockNum, threadNum, 0, 0>>>(d_s_kernel, d_s_potential_matrix,
				d_s_sub_index, rho, pathNum, d_Eu, M, 0);
