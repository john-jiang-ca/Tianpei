...
SNR=[8:2:18];       %signal to noie ratio (SNR) in (dB) 
...
ChannelRealization=1e3;   %the minimum number of channel realization
minSymbolError=150;       %the minimum number of symbol error accumulated 
%% Signal Modulation
...
fid=fopen('F:\GitHub\Tianpei\Channel Partition Code\Matlab\SEL-MMSE-OSIC\data\BER_SER_sel_MMSE_OSIC.txt', 'a');
fprintf(fid, '\n');
fprintf(fid, '=========================================================================================\n');
fprintf(fid ,'this file compare the BER simulation results of sel_MMSE-OSIC in LS(large scale)-MIMO systems\n');
...
[symOut_sel_MMSE_OSIC]=sel_MMSE_OSIC(sigRec, H, SNRd(count), M, pav, symConstell);   %MMSE-OSIC detection
% [symOut_sel_MMSE_OSIC]=MMSE_OSIC(sigRec, H, SNRd(count)/Nt, M, pav);
