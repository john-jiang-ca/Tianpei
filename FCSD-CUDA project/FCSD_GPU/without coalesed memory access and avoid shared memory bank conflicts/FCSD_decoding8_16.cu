...
__constant__ int d_list[MATRIX_SIZE];  __constant__ int d_s_sub_index[2*threadNum*blockNum];
...
//    cuComplex *R_Eu_share=(cuComplex*)malloc(Nt*sizeof(cuComplex));
   __shared__ cuComplex R_Eu_share[MATRIX_SIZE*threadNum];
...
//	    	 s_potential_matrix[IDC2D(count1,index*blockNum*threadNum+tx,pathNum)].y=(-d);
	...
	{
		 s_potential_matrix[IDC2D(count1,index*blockNum*threadNum+tx,pathNum)]=d_psymbolconstellation[d_s_sub_index[IDC2D((Nt-count1-1),(index*blockNum*threadNum+tx),pathNum)]];

	}
		R_Eu_share[IDC2D(tid,count1,MATRIX_SIZE)]=complex_sub( s_potential_matrix[IDC2D(count1,index*blockNum*threadNum+tx,pathNum)],d_constant_shat[count1]);
...
//		Eu_norm_share[tid]=complex_add(Eu_norm_share[tid],complex_mulcom(d_R[IDC2D(count1,count3,MATRIX_SIZE)],R_Eu_share[count3]));
			Eu_norm_share[tid]=complex_add(Eu_norm_share[tid],complex_mulcom(d_R[IDC2D(count1,count3,MATRIX_SIZE)],R_Eu_share[IDC2D(tid,count3,MATRIX_SIZE)]));
		}
		Eu_t[tx+index*blockNum*threadNum]=Eu_t[tx+index*blockNum*threadNum]+pow(Eu_norm_share[tid].x,2)+pow(Eu_norm_share[tid].y,2);
...
//	int *s_sub_index=(int*)calloc(1,rho*pow(M,rho)*sizeof(int));
	int *s_sub_index=(int*)malloc(rho*pathNum*sizeof(int));
...
//	cudaMemcpyAsync(s_potential_matrix,d_s_potential_matrix,MATRIX_SIZE*(pathNum)*sizeof(cuComplex),cudaMemcpyHostToDevice,0);
	FEpath<<<blockNum, threadNum,0,0>>>(d_s_kernel,rho,pathNum, M,count1);
