...
//		cuComplex *s_potential,   //the matrix use to store all the solution candidates from all the blocks
		...
		int rho, int pathNum, int loopNum,
//		cuComplex *s,  //decoding results
		float *Eu,  //Euclidean distance of one kernel
		float *Eu_total,  //Euclidean distance from all the block
		cuComplex *s_block,
...
#pragma unroll
							...
							complex_add(s_temp,
									complex_mulcom(
											...
											(complex_sub(
													...
													s_potential_matrix[IDC2D(count2,tx,loopNum)]))));
				...
				if (M == 2)   //BPSK
					...
					if (s_temp.x > 0) {
						s_potential_matrix[IDC2D(count1,tx,loopNum)].x = d;
						s_potential_matrix[IDC2D(count1,tx,loopNum)].y = 0;
					} else {
						s_potential_matrix[IDC2D(count1,tx,loopNum)].x = (-d);
						s_potential_matrix[IDC2D(count1,tx,loopNum)].y = 0;
					}
					...
					if (s_temp.y < 0) {
						s_potential_matrix[IDC2D(count1,tx,loopNum)].y = -d;
					} else {
						s_potential_matrix[IDC2D(count1,tx,loopNum)].y = d;
					}
					...
					d = sqrt(float(3) / (2 * (float) (Nt * (M - 1))));
					if (s_temp.x < (-2 * d)) {
						s_potential_matrix[IDC2D(count1,tx,loopNum)].x =
								(-3 * d);
					...
					if (s_temp.y < (-2 * d)) {
						s_potential_matrix[IDC2D(count1,tx,loopNum)].y =
								(-3 * d);
...
//					printf("the s_temp is %0.4f%+0.4fi ", s_temp.x,s_temp.y);
				...
				s_potential_matrix[IDC2D(count1,tx,loopNum)] =
						d_psymbolconstellation[s_sub_index[IDC2D((Nt-count1-1),(tx+index*loopNum),pathNum)]];

			}
			R_Eu_share[count1] = complex_sub(
					s_potential_matrix[IDC2D(count1,tx,loopNum)],
...
//		Eu_norm_share=complex_add(Eu_norm_share,complex_mulcom(d_R[IDC2D(count1,count3,MATRIX_SIZE)],R_Eu_share[count3]));
	...
	__syncthreads();
	//find the solution candidate in one block

	if (tid == 0) {
		__shared__ float Eu_mini_value;
		__shared__ int Eu_mini_index;
		Eu_mini_value = Eu[bid * threadNum];
		Eu_mini_index = 0;

		for (count1 = 0; count1 < threadNum; count1++) {
			if (bid * threadNum + count1 + index * loopNum < pathNum) {
				if (Eu[bid * threadNum + count1] < Eu_mini_value) {
					Eu_mini_value = Eu[bid * threadNum + count1];
					Eu_mini_index = bid * threadNum + count1;
				}
			}
			for (count1 = 0; count1 < MATRIX_SIZE; count1++) {
				s_block[IDC2D(bid,count1,MATRIX_SIZE)] =
						s_potential_matrix[IDC2D(count1,Eu_mini_index,loopNum)];
				Eu_total[bid] = Eu_mini_value;
			}
...
__global__ void MED_loop(float *Eu_block, cuComplex *s_block, //blockNum candiates from the kernel function in one loop
		float *Eu_total,    //the blockNum
		cuComplex *s_kernel, int index)    //find the MED solution in one loop
		{
	int count1;
	__shared__ int flag;
	__shared__ float Eu_mini_value;
	...
	if (index == 0) {
		Eu_mini_index = 0;
		Eu_mini_value = Eu_block[0];
		flag = 1;
	} else {
		Eu_mini_index = 0;
		Eu_mini_value = Eu_total[0];
	}
	for (count1 = 0; count1 < blockNum; count1++) {
		if (Eu_block[count1] < Eu_mini_value) {
			Eu_mini_value = Eu_block[count1];
	...
	if (flag == 1) {
		Eu_total[0] = Eu_mini_value;
		printf("now the Eu_mini is %0.4f\n", Eu_total[0]);
		for (count1 = 0; count1 < MATRIX_SIZE; count1++) {
			s_kernel[count1] = s_block[IDC2D(Eu_mini_index,count1,MATRIX_SIZE)];
		}
	}
}
__global__ void MED_total(float *Eu,   //the minimum Euclidean distance
...
//	s_sub=(cuComplex*)malloc(pow(M,rho)*rho*sizeof(cuComplex));   //all the possible full expansion sub vector
	...
	int loopNum = blockNum * threadNum;
...
//	int threadNum=ceil(pathNum/(blockNum*stride)); //determined by the path number
	float *d_Eu_total, *d_Eu_loop, *d_Eu_block;
	cuComplex *d_s_potential_matrix, *d_s_kernel, *d_s_block;
...
//	    error=cudaMalloc((void**) &d_R, MATRIX_SIZE*MATRIX_SIZE*sizeof(cuComplex));
	...
	error = cudaMalloc((void**) &d_s_potential_matrix,
			loopNum * Nt * sizeof(cuComplex));
	error = cudaMalloc((void**) &d_s_block,
			blockNum * MATRIX_SIZE * sizeof(cuComplex));
//		printf("%s\n",cudaGetErrorString(cudaGetLastError()));
//		cudaMemset(d_s_potential_matrix,0,pathNum*Nt*sizeof(cuComplex));
//		error=cudaMalloc((void**) &d_list, MATRIX_SIZE*sizeof(int));
	error = cudaMalloc((void**) &d_Eu_total, sizeof(float));
	error = cudaMalloc((void**) &d_Eu_loop, loopNum * sizeof(float));
	error = cudaMalloc((void**) &d_Eu_block, blockNum * sizeof(float));
//		printf("%s\n",cudaGetErrorString(cudaGetLastError()));
...
//   }
   ...
   cudaMemset(d_Eu_block,1e4,blockNum*sizeof(float));
//	cudaMemcpyAsync(s_potential_matrix,d_s_potential_matrix,MATRIX_SIZE*(pathNum)*sizeof(cuComplex),cudaMemcpyHostToDevice,0);
		FEpath<<<blockNum, threadNum, 0, 0>>>(d_s_potential_matrix,
				d_s_sub_index, rho, pathNum, loopNum, d_Eu_loop, d_Eu_block,
				d_s_block, M, count1);
//		MED_loop<<<1, 1>>>(d_Eu_block, d_s_block, d_Eu_total, d_s_kernel,
//				count1);
...
//	MED_total<<<1,1>>>(d_Eu_total,d_s_potential_matrix,d_s_kernel,pathNum);
//   cudaFree(Eu_mini);
