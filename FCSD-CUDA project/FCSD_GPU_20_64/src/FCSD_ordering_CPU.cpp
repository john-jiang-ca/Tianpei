...
void FCSD_ordering_CPU(cuComplex *pH, int *list, cuComplex *d_pH_permuted) {
...
//			    printf("\n");
	...
	for (count2 = 0; count2 < MATRIX_SIZE - 1; count2++) {
		...
		gsl_vector *diag_abs;
		gsl_complex a;
		Pprod = gsl_matrix_complex_calloc((MATRIX_SIZE - count2),
				(MATRIX_SIZE - count2));
		Pinv = gsl_matrix_complex_calloc((MATRIX_SIZE - count2),
				(MATRIX_SIZE - count2));
		pH_temp1 = gsl_matrix_complex_calloc(MATRIX_SIZE,
				(MATRIX_SIZE - count2 - 1));
		diag = gsl_vector_complex_calloc((MATRIX_SIZE - count2));
		diag_abs = gsl_vector_calloc((MATRIX_SIZE - count2));
		LU = gsl_matrix_complex_calloc((MATRIX_SIZE - count2),
				(MATRIX_SIZE - count2));
		...
		if (count2 == 0) {
			for (count1 = 0; count1 < MATRIX_SIZE; count1++) {
				for (count3 = 0; count3 < MATRIX_SIZE; count3++) {
					GSL_SET_COMPLEX(&a, pH[IDC2D(count1,count3,MATRIX_SIZE)].x,
							pH[IDC2D(count1,count3,MATRIX_SIZE)].y);
...
//						    	printf("\n");
//						    }
		gsl_blas_zgemm(CblasConjTrans, CblasNoTrans, alpha, pH_loop, pH_loop,
				beta, Pprod);
...
//				    printf("%0.4f%+0.4fi\n",gsl_matrix_complex_get(Pprod,0,0).dat[0],gsl_matrix_complex_get(Pprod,0,0).dat[1]);
		...
		for (count1 = 0; count1 < MATRIX_SIZE - count2; count1++) {
			for (count3 = 0; count3 < MATRIX_SIZE - count2; count3++) {
				//						    		pH_loop[IDC2D(count1,count3,MATRIX_SIZE)]=pH[IDC2D(count1,count3,MATRIX_SIZE)];
				gsl_matrix_complex_set(LU, count1, count3,
						gsl_matrix_complex_get(Pprod, count1, count3));
			}
		}
...
//				    	    printf("%0.4f%+0.4fi\n",gsl_matrix_complex_get(Pinv,0,0).dat[0],gsl_matrix_complex_get(Pinv,0,0).dat[1]);
		...
		for (count1 = 0; count1 < MATRIX_SIZE - count2; count1++) {
			...
			gsl_vector_complex_set(diag, count1,
					gsl_matrix_complex_get(Pinv, count1, count1));
...
//			        printf("this is for test\n");
		for (count1 = 0; count1 < MATRIX_SIZE - count2; count1++) {
			temp = sqrt(
					pow(gsl_vector_complex_get(diag, count1).dat[0], 2)
							+ pow(gsl_vector_complex_get(diag, count1).dat[1],
									2));
		...
		for (count3 = 0; count3 < MATRIX_SIZE; count3++) {
			for (count1 = 0; count1 < MATRIX_SIZE - count2; count1++) {
				if (count1 < (j)) {
					...
					gsl_matrix_complex_set(pH_temp1, count3, count1,
							gsl_matrix_complex_get(pH_loop, count3, count1));
				} else if (count1 > (j)) {
					//						 pH_temp1[IDC2D(count3,count1-1,(MATRIX_SIZE-count2-1))]=pH_loop[IDC2D(count3,count1,(MATRIX_SIZE-count2))];
					gsl_matrix_complex_set(pH_temp1, count3, (count1 - 1),
							gsl_matrix_complex_get(pH_loop, count3, count1));
				}
			}
		}
		gsl_matrix_complex_free(pH_loop);
		pH_loop = gsl_matrix_complex_calloc(MATRIX_SIZE,
				(MATRIX_SIZE - count2 - 1));
		for (count3 = 0; count3 < MATRIX_SIZE; count3++) {
			for (count1 = 0; count1 < MATRIX_SIZE - count2 - 1; count1++) {
				...
				gsl_matrix_complex_set(pH_loop, count3, count1,
						gsl_matrix_complex_get(pH_temp1, count3, count1));
			}
		}
	...
	pH_permuted1 = (cuComplex*) malloc(
			MATRIX_SIZE * MATRIX_SIZE * sizeof(cuComplex));
	for (count1 = 0; count1 < MATRIX_SIZE; count1++) {
		for (count2 = 0; count2 < MATRIX_SIZE; count2++) {
			pH_permuted1[IDC2D(count1,(MATRIX_SIZE-count2-1),MATRIX_SIZE)] =
