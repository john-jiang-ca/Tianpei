...
//		    	 gsl_matrix_complex_get(s_matrix_Q,count2,pathNum).dat[0]=float(int(S_temp.dat[0]/d)*d)+float(int(round(S_temp.dat[0]/d-int(S_temp.dat[0]/d)))*d);
//		    	 gsl_matrix_complex_get(s_matrix_Q,count2,pathNum).dat[1]=0;
			...
			else if(M==4)   //QPSK
			{
				...
				float *distance=(float*)calloc(1,M*sizeof(float));
		           ...
		           for(count3=0;count3<M;count3++)
		           {
		        	   ...
		        	   {
		        	   case 0:
		        	   distance[count3]=sqrt(pow(S_temp.dat[0]-(-d),2)+pow(S_temp.dat[1]-0,2)); break;
		        	   case 1:
		        	   distance[count3]=sqrt(pow(S_temp.dat[0]-0,2)+pow(S_temp.dat[1]-(-d),2)); break;
		        	   case 2:
		        	   distance[count3]=sqrt(pow(S_temp.dat[0]-(d),2)+pow(S_temp.dat[1]-0,2)); break;
		        	   case 3:
		        	   distance[count3]=sqrt(pow(S_temp.dat[0]-0,2)+pow(S_temp.dat[1]-d,2)); break;
...
//		                   #endif
                ...
                gsl_complex QPSK_temp;
		           switch (mini_index)
		           {
		           case 1:
		        	GSL_SET_COMPLEX(&QPSK_temp,-d,0);
		        	gsl_vector_complex_set(s_matrix_Q,count2,QPSK_temp);
		        	break;
		           case 2:
			      GSL_SET_COMPLEX(&QPSK_temp,0,-d);
			      gsl_vector_complex_set(s_matrix_Q,count2,QPSK_temp);
		           	break;
		           case 3:
			      GSL_SET_COMPLEX(&QPSK_temp,d,0);
...
//printf("the kernel time CPU is %f:\n", durationKernel);
//printf("the kernel clock is %f :\n", double(end-start));
//for(count4=0;count4<pathNum;count4++)
//{
//	printf("%0.4f ", gsl_vector_get(Eu,count4));
//}
//printf("\n");
//printf("the s_matrix list is:\n");
//for(count4=0;count4<pathNum;count4++)
//{
//	for(count3=0;count3<Nt;count3++)
//	{
//	printf("%0.4f%+0.4f ", gsl_matrix_complex_get(s_matrix_Q,count3,count4).dat[0],gsl_matrix_complex_get(s_matrix_Q,count3,count4).dat[1]);
//	}
//	printf("\n");
//}
//printf("\n");
//j=gsl_vector_min_index(Eu);
