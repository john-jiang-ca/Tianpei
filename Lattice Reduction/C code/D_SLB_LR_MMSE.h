...
#include"common.h"
#define e 2.71828
D_SLB_LR_MMSE(preceived,pH,SNR,Nr,Nt,M,psymbolconstellation,symOut)
{
...
gsl_blas_cgemm(CblasConjTrans,CblasNoTrans,one,T,I,zero,T_update );
gsl_blas_cgemm(CblasNoTrans,CblasNoTrans,one,pH,T_update,zero,pH_update);
...
gsl_matrix_complex_complex *wishart=gsl_matrix_complex_calloc(Nt,Nt);
gsl_matrix_complex *LU=gsl_matrix_complex_calloc(Nt,Nt);
gsl_matrix_complex *M_inverse=gsl_matrix_calloc(Nt,Nt);
gsl_matrix_complex *G=gsl_matrix_calloc(Nt,Nr);
gsl_vector_complex *x_hat=gsl_vector_calloc(Nt);
gsl_vector_complex *x_hat_q=gsl_vector_calloc(Nt);
...
gsl_blas_cgemm(CblasConjTrans,CblasNoTrans, one, pH_update,pH_update,one,I);
gsl_linalg_complex_LU_decomp(I,p,x);
gsl_linalg_complex_LU_invert(I,p,M_inverse);
gsl_blas_cgemm(CblasNoTrans,CblasConjTrans,one,M_inverse,pH_update,zero,G);
gsl_blas_cgemv(CblasNoTrans,one,G,preceived,zero,x_hat);
gsl_blas_cgemv(CblasNoTrans,one,T,x_hat,zero,x_hat_q);
//quantization
double temp;
d=sqrt(3/(2*double(Nt)*(M-1)));
for (count1=0;count1<Nt;count1++)
{

  temp=gsl_vector_get(x_hat_q,count1);
  if(temp.x>0)
  {
