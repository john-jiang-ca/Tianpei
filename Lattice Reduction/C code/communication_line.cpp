...
int main(void) {
	...
	SNR = (float*) malloc(SNRnum * sizeof(float));
	for (count1 = 0; count1 <=SNRnum; count1++) {
		SNR[count1] =pow(10, float(((count1) * 2) / float(10))); //test the SNR per bit from 2 to 20 the step is 2
	}
	float SNRbegin=0;      //the begining SNR point
	float SNRend=SNRnum   //the end SNR point
...
//	for(count1=0;count1<MATRIX_SIZE;count1++)
	...
	for(count1=SNRbegin;count1<=end;count1++)
	{
	printf("%d ", 2*count1);
	}
	printf("\n");
...
//			}
			...
			LR_BP=(preceived,pH,SNR[count],psymbolconstellation,symOut);    //LR aided belief propagation decoder
			LR_MMSE=(preceived,pH,SNR[count],psymbolconstellation,symOut);  //LR aided MMSE decoder
			end = clock();
