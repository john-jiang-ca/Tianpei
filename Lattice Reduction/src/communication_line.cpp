...
//   cudaMemset(Eu_mini,0,sizeof(double));
#include"modulator.h"
...
#include "common.h"
#define BER "BER_DSLBMMSE_test.txt"   //the file to store the bit error rate
#define SER "SER_DSLBMMSE_test.txt"  //the file to store the symbol error rate
...
int main(void) {
	...
	long int bitError=0;               //bit error 
	long int symbolError=0;            //symbol error
	...
	double *SNR;      //this is the SNR per bit in dB
	int M = Constellationsize;                               //modulation scheme
	int count, count1, count2, count3, count4;   //used for loops
	const int Nt = MATRIX_SIZE_Nt; // Nt is the number of transmit antennas. Must be a positive integer.
	const int Nr = MATRIX_SIZE_Nr; // Nr is the number of receive antennas. Must be a positive integer.
	unsigned long temp, errors;
	symbolError = 0;
	gsl_complex one;
	GSL_SET_COMPLEX(&one, 1.0, 0.0);
	gsl_complex zero;
	GSL_SET_COMPLEX(&zero, 0.0, 0.0);
	SNR = (double*) malloc((SNRnum+1) * sizeof(double));
	for (count1 = 0; count1 <=SNRnum; count1++) {
		SNR[count1] =pow(10, double(((count1) * 2) / double(10))); //test the SNR per bit from 2 to 20 the step is 2
	}
	double SNRbegin=SNRnum;      //the begining SNR point
	double SNRend=SNRnum;   //the end SNR point
...
//	for(count1=0;count1<MATRIX_SIZE;count1++)
	...
	double *durationKernel_CPU, duration_CPU;
	double *durationKernel_GPU, duration_GPU;
	double durationKernel_CPU_t = 0;
	double durationKernel_GPU_t = 0;
	durationKernel_CPU = (double*) malloc(sizeof(double));
	durationKernel_GPU = (double*) malloc(sizeof(double));
	printf("the program for %dX%d %d QAM SNRbegin!!\n", Nr,Nt,M);
	printf("the SNR per bit is:\n");
	//fprintf(cfile1,"this is the SNR %d %f\n", 2*SNRnum, SNR[SNRnum] );
	for(count1=SNRbegin;count1<=SNRend;count1++)
	{
	printf("%d ", 2*count1);
	}
	printf("\n");
	fprintf(cfile1,"\n");
	fprintf(cfile2,"\n");
	fprintf(cfile1,"D_SLB_MMSE decoder");
	fprintf(cfile2,"D_SLB_MMSE decoder");
	fprintf(cfile1, "this is the bit error rate of %d X %d MIMO \n",
			MATRIX_SIZE_Nr, MATRIX_SIZE_Nt);
...
//	fprintf(cfile2,"this is SNR %d %f\n", 2*SNRnum, SNR[SNRnum]);
	...
	fclose(cfile4);
	time1 = clock();
for (count = SNRbegin; count <=SNRend; count++) {
		iteration = 0;
		bitError = 0;
		symbolError = 0;
		BitErrorRate = 0;
...
//			gsl_vector_ulong_set(pgraydata,count2,gsl_vector_ulong_get(pdata,count2));   //find the gray
			...
			sigmas = sqrt(1.0 / Nt);
			sigman = sqrt(pow(sigmas, 2)*double(Nt) / (2*SNR[count])*double(log2(M))); /* corresponding noise standard deviation per dimension */
...
//			D_SLB_LR_BP=(preceived,pH,SNR[count],psymbolconstellation,symOut);    //LR aided belief propagation decoder
			D_SLB_LR_MMSE(preceived,pH,SNR[count],Nr, Nt, M,psymbolconstellation,symOut);  //LR aided MMSE decoder
			end = clock();
...
//			demodulatro_GPU(symOut_cu, psymbolconstellation, pgrayindexes, poutput); //poutput is the graycode of the symbols
//			for(count1=0;count1<Nt;count1++)
//			{
...
//		BitErrorRate=double(500)/double(iteration*MATRIX_SIZE*log2(M));
		BitErrorRate = double(bitError)/double(double(iteration)*MATRIX_SIZE_Nt*log2(M));
		SymbolErrorRate=double(symbolError)/double(double(iteration)*MATRIX_SIZE_Nt);
