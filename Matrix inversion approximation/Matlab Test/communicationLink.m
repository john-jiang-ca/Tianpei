...
SNR=[30:2:50];       %average receive signal to noie ratio (SNR) in (dB) 
...
SER_MMSE_E=zeros(length(SNR),1);    %symbol error rate of MMSE detector with exact matrix inversion for every SNR (dB) 
BER_MMSE_E=zeros(length(SNR),1);    %bit error rate of MMSE detector with exact matrix inversion for every SNR (dB)
SER_MMSE_A=zeros(length(SNR),1);    %symbol error rate of MMSE detector with SENIA approximation for every SNR (dB) 
BER_MMSE_A=zeros(length(SNR),1);    %bit error rate of MMSE detector with SENIA approximation for every SNR (dB)
minRealization=1e4;   %the minimum number of channel realization
...
fid=fopen('F:\GitHub\Tianpei\Matrix inversion approximation\Matlab Test\data\MMSE_SENIA_test.txt', 'a');
fprintf(fid, '\n');
fprintf(fid, '=========================================================================================\n');
fprintf(fid ,'this routine tests the performance of the series expansion Newton Iteration matrix approximation for Linear Detectors\n');
...
fprintf(fid, '****************\n');
fprintf(fid, 'MMSE\n');
fprintf(fid, 'The expectation of OM is %e\n', Eom);
...
fprintf(fid, '-------------\n');
fprintf(fid, 'MMSE with exact matrix inversion\n');
...
for count=1:length(SNR)    
    symError_MMSE_E=0;  %record the symbol error of MMSE E
    bitError_MMSE_E=0; %record the bit error of MMSE E
    symError_MMSE_A=0;  %record the symbol error of MMSE E
    bitError_MMSE_A=0; %record the bit error of MMSE E
    Realization=0;    %number of channel realization
while(symError_MMSE_E<minSymbolError||Realization<minRealization)
...
[symOut_MMSE_E]=MMSE(sigRec, H, SNRd(count)/Nt, M, pav);   %MMSE detection with exact matrix inversion
[symOut_MMSE_A]=MMSE_A(sigRec, H, SNRd(count)/Nt, M, pav); %MMSE detection with SENIA approximation
symError1_v=abs(dataMod-symOut_MMSE_E);
ErrorIndex=find(symError1_v>1e-5);
symError_MMSE_E=symError_MMSE_E+length(ErrorIndex);  %calculate the symbol error of MMSE in this channel realization
bitOut1=grayDecoder(symOut_MMSE_E, ErrorIndex, graycode, symConstell);  %decoding the gray code of the output symbol vector of MMSE-OSIC
bitError_MMSE_E=bitError_MMSE_E+checkBitError(bitOut1, ErrorIndex, grayData, M);% calculate the bit error of MMSE-OSIC in this channel realization

symError1_v=abs(dataMod-symOut_MMSE_A);
ErrorIndex=find(symError1_v>1e-5);
symError_MMSE_A=symError_MMSE_A+length(ErrorIndex);  %calculate the symbol error of MMSE in this channel realization
bitOut1=grayDecoder(symOut_MMSE_A, ErrorIndex, graycode, symConstell);  %decoding the gray code of the output symbol vector of MMSE-OSIC
bitError_MMSE_A=bitError_MMSE_A+checkBitError(bitOut1, ErrorIndex, grayData, M);% calculate the bit error of MMSE-OSIC in this channel realization


Realization=Realization+1;  %calculate the number of channel realizations
 end
% Calculate the SER and BER
SER_MMSE_E(count)=symError_MMSE_E/(Realization*Nt);  %caculate symbol error rate of MMSE E
BER_MMSE_E(count)=bitError_MMSE_E/(Realization*Nt*ceil(log2(M))); %calculate bit error rate of MMSE E
SER_MMSE_A(count)=symError_MMSE_A/(Realization*Nt);  %caculate symbol error rate of MMSE A
BER_MMSE_A(count)=bitError_MMSE_A/(Realization*Nt*ceil(log2(M))); %calculate bit error rate of MMSE A
fprintf(fid,'%d \t %d \t %d \t %d \t %e \t %e\n', SNR(count), Realization, symError_MMSE_E, bitError_MMSE_E, SER_MMSE_E(count), BER_MMSE_E(count));
end
fprintf(fid, '\n');
fprintf(fid, '-------------\n');
fprintf(fid, '\n');
fprintf(fid, '-------------\n');
fprintf(fid, 'MMSE with SENIA approximation\n');
fprintf(fid, 'SNR \t iteration \t symbol errors \t  bit errors \t SER \t BER\n');
for count=1:length(SNR)
    fprintf(fid,'%d \t %d \t %d \t %d \t %e \t %e\n', SNR(count), Realization, symError_MMSE_A, bitError_MMSE_A, SER_MMSE_A(count), BER_MMSE_A(count));
end
fprintf(fid, '-------------\n');
fprintf(fid, '\n');
fprintf(fid, '****************\n');
