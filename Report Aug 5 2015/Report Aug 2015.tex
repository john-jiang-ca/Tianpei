...
\end{eqnarray}
where $\eta$,$\hat{\eta}$, $\alpha$, $\hat{\alpha}$ are Lagrange multipliers, the solution of dual optimization problem is saddle point, from \cite{saddle point}, the dual problem of (\ref{lagrange duality1}) can be formulated as
\begin{eqnarray}
\nonumber
...
\end{eqnarray}
Karush–Kuhn–Tucker (KKT) condition is satisfied or not \cite{} determines whether a non linear optimization problem find its optimal solution, the complementary KKT solution of (\ref{lagrange duality2}) is 
\begin{IEEEeqnarray}[\relax]{l}
\left\{\begin{array}{ll}
\eta_{i}\xi_{i}=0\\
\hat{\eta}_{i}\hat{\xi}_{i}=0\\
\alpha_{i}(\epsilon+\xi_{i}-y_{i}+\mathbf{w}^{T}\Phi(\mathbf{x}_{i}))=0\\
\hat{\alpha}_{i}(\epsilon+\hat{\xi}_{i}+y_{i}-\mathbf{w}^{T}\Phi(\mathbf{x}_{i}))=0\\
\end{array}\right.
...
\end{IEEEeqnarray}
where $(\alpha-\hat{\alpha})$, $(\beta-\hat{\beta})$,$Re(\mathbf{y})$, $Im(\mathbf{y})$ denote vectors, $\mathbf{e}=[1,1,\ldots, 1]^{T}\in \mathbb{R}^{N_{r}}$, $\mathbf{K}^{r}$ denotes the matrix consist of real  part of kernel components. 
...
Because (\ref{complex duality real part}) and (\ref{complex duality imaginary part}) are symmetric, in this section we discuss real part only. By dividing the variables into work set $S$ and fixed set $N$, (\ref{complex duality real part vectorizes}) can be changed to:
\begin{IEEEeqnarray}[\relax]{l}
\nonumber
...
\begin{equation}
maximize\quad \Theta^{r}=-\frac{1}{2}[(\alpha-\hat{\alpha})_{S}^{T}\mathbf{K}_{SS}^{r}(\alpha-\hat{\alpha})_{S}]-[(\alpha-\hat{\alpha})_{N}^{T}\mathbf{K}_{NS}^{r}
+Re(\mathbf{y})_{S}^{T}](\alpha-\hat{\alpha})_{S}-\epsilon[\mathbf{e}_{S}^{T}(\alpha+\hat{\alpha})_{S}],
...
\subsection{Single Direction Solver}
 ...
 \begin{IEEEeqnarray}[\relax]{l}
 \nonumber
 maximize\quad \Theta_{1}^{r}=-\frac{1}{2}((\alpha_{1}-\hat{\alpha}_{1})^{new})^{2}\mathbf{K}_{11}^{r}-(\alpha_{1}-\hat{\alpha}_{1})^{new}\sum_{j=2}^{N_{r}}\mathbf{K}_{1j}^{r}(\alpha_{j}-\hat{\alpha}_{j})^{new}
...
\end{IEEEeqnarray}
Based on (\ref{partial optimization function 1-D alpha}), (\ref{partial optimization function 1-D alpha hat}) and (\ref{clipped new dual variable}), The gain of objective function respect to $i$th dual variable pair is 
\begin{IEEEeqnarray}[\relax]{l}
\nonumber
\bigtriangledown \Theta_{i}^{r}=\Theta^{r}((\alpha+\delta_{i}\mathbf{e}_{i}), (\hat{\alpha}+\hat{\delta}_{i}\mathbf{e}_{i}))-\Theta^{r}(\alpha, \hat{\alpha})\\
\nonumber
=-\frac{1}{2}(\delta_{i}-\hat{\delta}_{i})^{2}\mathbf{K}_{ii}^{r}-(\delta_{i}-\hat{\delta}_{i})\sum_{j=1}^{N_{r}}(\alpha_{j}-\hat{\alpha}_{j})\mathbf{K}_{ij}^{r}+Re(y_{i})(\delta_{i}-\hat{\delta}_{i})-\epsilon(\delta_{i}+\hat{\delta}_{i})\\
...
\label{gain of objective function}
\end{IEEEeqnarray}
