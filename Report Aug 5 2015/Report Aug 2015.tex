...
The sufficient and necessary condition that $f(\mathbf{w}^{*})$ is the global minimum of (\ref{primal objective function}) is called Karush Kuhn Tucker conditions, which can be written as 
\begin{IEEEeqnarray}[\relax]{l}
\frac{\partial \Theta}{\partial \mathbf{w}}=w-\sum_{i=1}^{l}(\alpha_{i}-\hat{\alpha}_{i})\\\label{partial1}
...
\end{array}\right. \label{KKT complimentary}
\end{IEEEeqnarray}
 
The conditions in (\ref{KKT complimentary}) is called KKT complimentary condition. Then substitute (\ref{partial1})-(\ref{partial4}) to (\ref{lagrange duality2}) and for sake of brevity, we make $C_{i}$ uniform to all data samples, we have the dual form ：
\begin{IEEEeqnarray}[\relax]{l}
\nonumber
...
\end{IEEEeqnarray}
Obviously $\theta(\alpha,\hat{\alpha})\leq \Theta(\mathbf{w},\alpha, \hat{\alpha}, \eta, \hat{\eta})$, Because 
\begin{equation}
