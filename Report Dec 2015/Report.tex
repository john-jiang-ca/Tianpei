...
\begin{abstract}
This report consists of two major parts. In the first part, based on the performance analysis of list based algorithm, minimum achievable diversity (MAD) channel selection principle for Large-Scale MIMO (LS-MIMO) systems is presented. MAD principle can reduce the detection complexity by sacrificing redundant asymptotic diversity gain in LS-MIMO. Computer simulation results for the performance of MAD principle are presented for different MIMO system configurations. In the second part, theoretical analysis are provided for channel hardening phenomenon, including logarithmic expectation and probability distribution of orthogonality measure, which is defined to evaluate the orthogonality of the channel matrix. 
\end{abstract}
