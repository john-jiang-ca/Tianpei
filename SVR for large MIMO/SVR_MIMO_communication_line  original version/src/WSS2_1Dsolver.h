...
for(count=0;count<2*Nr;count++){
	if(count<Nr){
		temp_l_m=gsl_vector_get(l_m, count)+(phi_p[count]-epsilon)/
				(gsl_matrix_get(R_Kernel, count,count));
		if(temp_l_m<0){
			temp_l_m=0;
		}else if (temp_l_m>C){
			temp_l_m=C;
		...
		temp_l_m_hat=gsl_vector_get(l_m, count)-(phi_p[count]+epsilon)/
				(gsl_matrix_get(R_Kernel, count,count));
		if(temp_l_m<0){
			temp_l_m_hat=0;
		}else if (temp_l_m>C){
	...
	g=abs((sigma-sigma_hat)(-0.5*(sigma-sigma_hat)*gsl_matrix_get(R_Kernel(count,count))+
			phi_p[count]-epsilon*(sigma+sigma_hat)/(sigma-sigma_hat)));
	if(g>best_g){
		best_g=g;
		S_i=F_i;
		F_i=count;
