...
SNR=[10:2:20];       %signal to noise ratio per bit in dB
...
pav=1;  %average symbol power
[symConstell]=symbolConstellation( M, pav );

%% generate files
fid_t=fopen('F:\GitHub\Tianpei\SVR for large MIMO\real SVR matlab\CSVR\MFB\data\README.txt','a');   %the file for explanations
fprintf(fid_t, 'this is %d X %d MIMO\n', Nr, Nt);
fprintf(fid_t, '%d QAM modulation\n', M);
fprintf('\n');
fprintf(fid_t, 'The detectors tested are\n');
fprintf(fid_t, 'MFB-SER1\n');
...
fid1=fopen('F:\GitHub\Tianpei\SVR for large MIMO\real SVR matlab\CSVR\MFB\data\SER1.txt','a');  %the file store the SER values
...
fprintf(fid1, 'symbol error rate of MFB\n');
...
% fid6=fopen('SER6.txt','a');
% fid7=fopen('SER6.txt','a');
 while(symError1<200||channelRealization<5e4)
% symOut=zeros(Nt,1)
% for k=1:symNum/Nt
% for i=1:Nt
%     for j=1:Nr
...
%   [symOut3]=MMSE_OSIC(sigRec, H, SNRd(count), M, pav);
...
SER1(count)=symError1/(channelRealization*Nt);  %caculate symbol error rate of MFB
...
fid_t=fopen('F:\GitHub\Tianpei\SVR for large MIMO\real SVR matlab\CSVR\MFB\data\README.txt', 'a');
