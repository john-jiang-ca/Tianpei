...
SNR=[10:2:26];       %signal to noise ratio per bit in dB
SNRd=10.^(SNR.*0.1);   %SNR in dicimal
noiseV=1./SNRd;   %noise variance of AWGN 
BER1=zeros(length(SNR),1);         %bit error rate
...
graycode=grayEncoder(M);
%% generate files
fid_t=fopen('/home/tchen44/code/Tianpei/SVR for large MIMO/real SVR matlab/CSVR/MMSE/128/4QAM/data/README.txt','a');   %the file for explanations
fprintf(fid_t, '\n');
fprintf(fid_t, '-----------------\n');
...
fid1=fopen('/home/tchen44/code/Tianpei/SVR for large MIMO/real SVR matlab/CSVR/MMSE/128/4QAM/data/BER1.txt','a');  %the file store the SER values
...
fprintf(fid1, 'symbol error rate of MMSE\n');
...
fprintf(fid1,'\n');
fprintf(fid_t, '\n');
fprintf(fid1, 'the bit error rates are\n');
% fprintf(fid2, '\n');
% fprintf(fid3, '\n');
% fprintf(fid4, '\n');
% fprintf(fid5, '\n');
...
% fid1=fopen('/home/tchen44/code/Tianpei/SVR for large MIMO/real SVR matlab/CSVR/MMSE/32/4QAM/data/SER1.txt','a');  %the file store the SER values
% fid2=fopen('SER2.txt','a');
% fid3=fopen('SER3.txt','a');
% fid4=fopen('SER4.txt','a');
% fid5=fopen('SER5.txt','a');
...
BER1(count)=bitError1/(channelRealization*Nt*log2(M)); %calculate bit error rate of MMSE
...
fprintf(fid1, '\n');
fprintf(fid1, 'the program ends successfully!\n');
fprintf(fid1, '-----------------\n');
fid_t=fopen('/home/tchen44/code/Tianpei/SVR for large MIMO/real SVR matlab/CSVR/MMSE/128/4QAM/data/README.txt', 'a');
