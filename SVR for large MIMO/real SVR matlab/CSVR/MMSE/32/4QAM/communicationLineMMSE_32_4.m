...
SER=zeros(length(SNR),1);  %symbol error rate of MMSE
BER=zeros(length(SNR),1);  %bit error rate of MMSE
MSE_V=zeros(length(SNR),1);   %MSE of MMSE
global C;
global tol;
global epsilon;
C=4;
...
%     hBER = comm.ErrorRate;
 fid=fopen('/home/tchen44/code/Tianpei/SVR for large MIMO/real SVR matlab/CSVR/MMSE/32/4QAM/data/MSE.txt', 'a');
 fprintf(fid, '\n');
 fprintf(fid, '-----------------\n');
 fprintf(fid ,'this file record the MSE results of MMSE\n');
%  fprintf(fid, 'RSVD\n');
...
[symOut1, MSE_tmp]=MMSE( sigRec, H, SNRd(count), M, pav, dataMod);
