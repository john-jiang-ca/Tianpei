...
SNR=[4:2:22];       %signal to noise ratio per bit in dB
...
%     hBER = comm.ErrorRate;
 fid=fopen('/home/tchen44/code/Tianpei/SVR for large MIMO/real SVR matlab/CSVR/RSVD-GA/32/16QAM/data/BER_RSVD_GA.txt', 'a');
 fprintf(fid, '\n');
 fprintf(fid, '-----------------\n');
...
%     bitOutput=zeros(nBits,1);
 fid=fopen('//home/tchen44/code/Tianpei/SVR for large MIMO/real SVR matlab/CSVR/RSVD-GA/32/16QAM/data/BER_RSVD_GA.txt', 'a');
 while(symError1<150||channelRealization<2e3)
% symOut=zeros(Nt,1);
% for k=1:symNum/Nt
% for i=1:Nt
%     for j=1:Nr
