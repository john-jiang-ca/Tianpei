function [index1,index2, sigma1, sigma2, sigma1_a, sigma2_a, Lambda_new, Phi_new, Psi_new, deltaTheta]=a2DSolver(Kr,Ki, Lambda, Phi, Psi, epsilon, label)
...
%% perform 2D update
danomin_tmp=(Kr(index1,index1)*Kr(index2,index2)-(Kr(index1,index2)^(2)));  %different from the original
...
deltaTheta=-0.5*(sigma1^(2)*Kr(index1,index1)+sigma2^(2)*Kr(index2,index2)+2*sigma1*sigma2*Kr(index1,index2))...
    +Phi(index1)*sigma1+Phi(index2)*sigma2-epsilon*(sigma1_a+sigma2_a);
Phi_new=Phi-sigma1*Kr(:,index1)-sigma2*Kr(:,index2);
if(label==1)
