...
%     hBER = comm.ErrorRate;
 fid=fopen('/home/tchen44/code/Tianpei/SVR for large MIMO/real SVR matlab/CSVR/test data/SER_CSVD.txt', 'a');
 fprintf(fid, '-----------------\n');
 fprintf(fid, 'The result for complex SVD\n');
fprintf(fid, 'this is the test data for %d X %d MIMO system with %d QAM modulation\n', Nr,Nt,M);
fprintf(fid, 'the hyperparameters for CSVD are\n');
...
[symOut1]=CSVD(sigRec, H, SNRd(count), M);
