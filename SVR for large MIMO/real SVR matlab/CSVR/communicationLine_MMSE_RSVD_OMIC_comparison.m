...
%     hBER = comm.ErrorRate;
 fid=fopen('/home/tchen44/code/Tianpei/SVR for large MIMO/real SVR matlab/CSVR/test data/SER_RSVD_MMSE_OMIC.txt', 'a');
 fprintf(fid, '\n');
 fprintf(fid, '-----------------\n');
...
symError1=symError1+length(find(symError1_v>1e-4));
%  symError2_v=abs(dataMod-symOut2);
% symError2=symError2+length(find(symError2_v>1e-4));
% end
% end
