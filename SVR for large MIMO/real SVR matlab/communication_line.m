...
SNR=[4,6,8,10,12,14,16,18];  %signal to noise ratio of output
noiseVariance=zeros(length(SNR),1); %noise variance
...
%               M,d);
[symOut1, lamida, Theta, G, iteration, MSE1, NumReliable]=real_SVR_WSSS1D_2Dsolver(pH, y, SNRd(count), symbolReal, Nr, Nt, M,d,epsilon, C, tol,tau);
% I=eye(Nt,Nt);
% Dia=diag(I/(pH'*pH));
% 
% numberR=0;
...
% symOut2=zeros(Nt,1);
   ...
   SymErrorRate1(count)=symError1*(1/(channel_realization*Nt));
