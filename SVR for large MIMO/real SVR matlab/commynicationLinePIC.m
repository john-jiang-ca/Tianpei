...
SNR=[18];       %signal to noise ratio per bit in dB
...
%     bitOutput=zeros(nBits,1);
 ...
 while(symError1<200||channelRealization<1e4)
symOut=zeros(Nt,1);
% for k=1:symNum/Nt
% for i=1:Nt
%     for j=1:Nr
...
% end
% symOut=dataMod;
%   [symOut1]=PIC(sigRec, H, SNRd(count),M,pav);
  stage=1;
  maxStage=3;
  symOut1=zeros(Nt,1);
  W=ones(Nt,Nt);
  ...
  list=1:Nt;
  symOut_prev=[];
  [symOut3]=Partial_MIC_recursive( sigRec, H, symOut_total, list, H1, symOut_prev, SNRd, M, pav, stage, maxStage, symConstell, tol);
...
SER1(count)=symError1/(channelRealization*Nt);  %caculate symbol error rate of PIC
